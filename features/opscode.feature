Feature: Opscode Chef Server support
  ...
  Scenario Outline: deploy Chef Server configured nodes
    Given a Formatron project
    And a credentials.json file with content
      """
      {
    ...
    And a cloudformation/main.json file with content
      ...
      """
    And an opscode/node_1/metadata.rb file with content
      """
      name 'node_1'
      """
    And an opscode/node_2/metadata.rb file with content
      """
      name 'node_2'
      """
    And a <target>/chef-server/opscode/keys/<user>.pem S3 key with content
      """
      <user key>
      """
    When I deploy the formatron stack with target <target>
