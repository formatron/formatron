...
describe Formatron::Util::Berks do
  ...
  describe '::vendor' do
    ...
    context 'when not adding the lock file to the vendored cookbooks' do
      before(:each) do
        allow(@kernel_helper_class).to receive(:success?) { true }
        Formatron::Util::Berks.vendor('cookbook', 'vendor')
      end
    ...
    context 'when an error is encountered vendoring cookbooks' do
      ...
      it 'should fail' do
        expect { Formatron::Util::Berks.vendor('cookbook', 'vendor') }
          .to raise_error(Formatron::Util::Berks::VendorError)
      end
    end
  end
  ...
  context 'when verifying SSL certs' do
    before(:each) do
      @knife = Formatron::Util::Berks.new(
        'http://server',
        'user',
        'key',
        'organization',
    ...
    context 'when the berks install fails' do
      ...
      it 'should fail' do
        expect { @berks.upload_environment 'cookbook', 'environment' }
          .to raise_error(
            Formatron::Util::Berks::UploadEnvironmentError,
            'failed to download cookbooks for opscode environment: environment'
          )
      end
    end
    ...
    context 'when the berks upload fails' do
      ...
      it 'should fail' do
        expect { @berks.upload_environment 'cookbook', 'environment' }
          .to raise_error(
            Formatron::Util::Berks::UploadEnvironmentError,
            'failed to upload cookbooks for opscode environment: environment'
          )
      end
    end
